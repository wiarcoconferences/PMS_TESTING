...
def task_add(request):
    ...
    project_ids = user_id2.project.all()
    assigned_to = list(set(UserProfile.objects.filter(project__in=project_ids)))
...
def add_multiple_tasks(request):
    f = Task_Formf(int(request.user.pk))
    ...
    user_id2 = UserProfile.objects.get(user__id=user_id)
    project_ids = user_id2.project.all()
    assigned_to = UserProfile.objects.filter(project__in=project_ids)
    mid = request.GET.get('milestone_id')
    if mid:
        try:
            mobj = Milestone.objects.get(id=int(mid))
            ...
            pass
    form = f(prefix="task")
    subform = f(prefix="tas")
    subbform = f(prefix="tasks")
    error = ''
    msg = ''
    nam = ''
    x = ''
    ...
    messages = ''
    if request.method == 'POST':
        form = f(request.POST or None , request.FILES, prefix="task")
        subform = f(request.POST or None, request.FILES, prefix="tas")
        subbform = f(request.POST or None,request.FILES, prefix= "tasks")
...
@login_required(login_url="/login/")
def _filter_function(request, task_list):
    ...
    if followed_by_id:
        task_list = task_list.filter(followed_by__id=followed_by_id)
...
def task_overdue(request):
    ...
    overdue_list = Tasks.objects.filter(due_date__lt=datetime.date.today(), owned_by__user=request.user).exclude(status__name='Closed')
    return _filter_function(request, overdue_list)

def task_assigned(request):
    return _filter_function(request, Tasks.objects.filter(assigned_to__user=request.user))

def task_ownedby(request):
    return _filter_function(request, Tasks.objects.filter(owned_by__user=request.user))

def task_following(request):
    return _filter_function(request, Tasks.objects.filter(followed_by__user=request.user))

def task_home(request):
    for i in request.user.userprofile_set.all():
        if i.access_level == '1':
            return _filter_function(request, Tasks.objects.all())
    return task_ownedby(request)
...
def task_edit(request, id_edit):
    ...
    f = Task_Formf(request.user.pk)
    form = f(instance=task)
    user_id=request.user.id
    user_id2 = UserProfile.objects.get(user__id=user_id)
    if request.POST:
        form = f(request.POST, request.FILES, instance=task)
        if form.is_valid():
...
@login_required(login_url="/login/")
def _task_following(request):
    clients = Client.objects.all()
    modules = Modules.objects.all()
    task_list = Tasks.objects.all()
    uid_list = [profile.id for profile in request.user.userprofile_set.all()]
...
def task_request_add(request):
    ...
    messages = ''
    form = Request_Formf(request.user.pk)
    if request.method == 'POST':
        form = form(request.POST, request.FILES)
...
def task_request_edit(request, id_edit):
    ...
    f = Request_Formf(request.user.pk)
    form = f(instance = requests)
    doc = request.POST.get('doc')
    if request.POST:
        form = form(request.POST,request.FILES, instance = requests)
        if form.is_valid():
            f = form.save(commit=False)
            f.save()
            edit_done = True
